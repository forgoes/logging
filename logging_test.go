...
func TestTag(t *testing.T) {
	...
	SetTag("service", "logging")
	assert.Equal(t, 1, len(Tags()))
	assert.Equal(t, "logging", Tags()["service"])
...
func TestKv(t *testing.T) {
	...
	SetKv("service", "logging")
	assert.Equal(t, 1, len(Kvs()))
	assert.Equal(t, "logging", Kvs()["service"])
...
func TestError(t *testing.T) {
	...
	t.Run("emit Error log and enable frame", func(t *testing.T) {
		...
		m.On("Handle", mock.MatchedBy(func(e *Event) bool {
			return e.GetLogger() == root && e.level == ERROR &&
				e.msg == "" &&
				e.extra == nil && e.caller.pc > 0 && strings.Contains(e.caller.file, "logging_test.go") &&
				e.caller.ok == true
		})).Return(true, nil)
		m.On("Flush").Return(nil)
		m.On("Close").Return(nil)
