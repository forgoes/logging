...
func TestNilEvent(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		...
		e.Kv("k", "v").E(nil).Logf("")
		e.Kv("k", "v").E(nil).ALogf("%d,%d,%d\n", 1, 2, 3)
		e.Kv("k", "v").E(nil).ALogf("", 1, 2, 3)
	})
}
...
func TestEventWithCaller(t *testing.T) {
	...
	assert.True(t, e.GetCaller().GetFunc() == "github.com/forgoes/logging.TestEventWithCaller")
}
